...
- **Anthropic Messages** — `/v1/messages`，流式 + 非流式
- **OpenAI Responses** — `/v1/responses`，流式 + 非流式
- **动态模型列表** — `/v1/models`，从上游 `/v2/config` 获取并缓存
- **OAuth2 Device Flow** — 浏览器扫码登录，Token 自动持久化
...
curl "http://localhost:8000/auth/poll?auth_state=xxx"
# 登录成功后 Token 自动保存到 .codebuddy_creds/token.json
```

也可以手动设置 Token：

```bash
curl -X POST http://localhost:8000/auth/manual \
...
│   ├── config/config.go            # 配置
│   ├── auth/
│   │   ├── token.go                # Token 持久化 + JWT 解析
│   │   └── handler.go              # OAuth2 路由 + 上游请求头
│   └── proxy/
│       ├── handler.go              # 路由注册 + 请求处理
│       ├── stream.go               # OpenAI Chat SSE 流式转发
│       ├── models.go               # 动态模型列表
