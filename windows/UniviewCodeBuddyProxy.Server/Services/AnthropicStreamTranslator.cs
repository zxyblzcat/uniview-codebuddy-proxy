...
public sealed class AnthropicStreamTranslator
{
    ...
    public List<string> ProcessLine(string line)
    {
        ...
        foreach (var choice in choices.EnumerateArray())
        {
            ...
            {
                _started = true;
                result.Add(AnthropicSSE("message_start", new Dictionary<string, object?>
                {
                    ["type"] = "message_start",
            ...
            if (delta.TryGetProperty("reasoning_content", out var rcEl))
            {
                ...
                if (!string.IsNullOrEmpty(reasoningContent))
                {
                    if (_thinkingBlockIdx == null)
                    {
                        ...
                        result.Add(AnthropicSSE("content_block_start", new Dictionary<string, object>
                        {
                    ...
                    result.Add(AnthropicSSE("content_block_delta", new Dictionary<string, object>
                    {
            ...
            if (delta.TryGetProperty("content", out var contentEl))
            {
                ...
                if (!string.IsNullOrEmpty(content))
                {
                    ...
                    {
                        result.AddRange(EmitThinkingSignatureDelta(_thinkingBlockIdx.Value));
                        result.Add(AnthropicSSE("content_block_stop", new Dictionary<string, object>
                        {
            ...
            {
                _finished = true;
                result.AddRange(CloseOpenBlocks());
                result.Add(AnthropicSSE("message_delta", new Dictionary<string, object?>
                {
                    ["type"] = "message_delta",
                    ["delta"] = new Dictionary<string, object?> { ["stop_reason"] = FinishReasonToStopReason(fr), ["stop_sequence"] = null },
                    ["usage"] = new Dictionary<string, int> { ["input_tokens"] = _inputTokens, ["output_tokens"] = _outputTokens },
                }));
                result.Add(AnthropicSSE("message_stop", new Dictionary<string, object> { ["type"] = "message_stop" }));
            }
        }
